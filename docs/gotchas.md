...
`**這樣。**接著寫` 渲染出來是字面的 `**`，不是粗體。

CommonMark 判斷結尾的 `**` 能不能閉合，看它前後兩個字元：前面是標點（`。`）、
後面接的又不是空白或標點（漢字），這個 `**` 就不算 right-flanking，於是不閉合。
英文詞間有空格，幾乎碰不到；**中文每次都會碰到**。

寫成 `**這樣**。接著寫` 即可。

實測：凡例那一頁第一版有 7 處中招，全是同一個形狀（`。**` 後面接漢字）。
找法是渲染完之後在 HTML 裡搜還活著的字面 `**` —— 原始碼看不出來，因為原始碼本來就有 `**`。

## 程式碼區塊在夜行模式下看不見

Shiki 只設一個 `github-light` 時，每個字的顏色是寫死在 inline style 裡的深色，
而 `scroll.css` 用 `background: var(--panel) !important` 讓底色跟著主題走。
夜行下就是深色字疊深色底，量到 **1.18:1**。用淺色模式的人永遠看不到。

現在是雙主題（`github-light-high-contrast` / `github-dark`）：每個字同時帶 inline
的淺色與 `--shiki-dark` 變數，`scroll.css` 依站上的深色選擇器切換。淺色那邊不用
一般的 `github-light`，因為它的橘色 `#E36209` 在陳紙底 `#E8E3D6` 上只有 2.72:1 ——
那個主題是為白底設計的。

`e2e/contrast.spec.ts` 在兩個主題下各量一次：本文 ≥ 4.5、最淡的 token ≥ 3。

**反向驗證要兩邊一起還原**：只把設定改回單主題、留著 `var(--shiki-dark)` 的 CSS，
`var()` 失效後 `color` 會退回繼承正文色，深色反而讀得到 —— 測試照綠，驗不到東西。
